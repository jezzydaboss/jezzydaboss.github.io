...
Working this out in pseudocode first is what surfaced the trickiest design decision: whether a resize's internal rehashing should count toward the collision total. Counting it would have made the collision metric misleading, so the pseudocode explicitly separates the two concerns before any Java was written.
