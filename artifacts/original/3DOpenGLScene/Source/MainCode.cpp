...
// Callback prototypes (declared so registration compiles)
...
int main(int argc, char* argv[])
{
   ...
   // Ensure the OpenGL context is current before initializing GLEW
	...
	glfwSetInputMode(g_Window, GLFW_CURSOR, GLFW_CURSOR_DISABLED); // optional FPS-style
	...
	// CRITICAL: Pass the window to SceneManager so it can read WASD keys
	g_SceneManager->SetWindow(g_Window);
	...
	while (!glfwWindowShouldClose(g_Window))
	{
		...
		// Process input after polling events
